...
@MainActor
@Observable class MoneyBreakViewModel {
    ...
    var localPlayerName: String = "Kamu"
    var remotePlayerName: String = "Teman"
    ...
    func fetchPlayerNames() {
        ...
        if let match = parent?.matchManager?.myMatch,
           let otherPlayer = match.players.first(where: { $0.gamePlayerID != GKLocalPlayer.local.gamePlayerID }) {
            self.remotePlayerName = otherPlayer.alias
        }
    }
    ...
    func onButtonTapped() {
        guard hasMoneyAllocated else { return }
        toggleReady()
    }
